...
The trigger is hidden with `invisible` when there are no tabs, **never unmounted**, and the earlier `ResizeObserver`-driven "only show it when overflowing" was dropped for the same reason: closing the LAST tab from inside the popover would unmount an open Radix layer, which is the stuck `pointer-events: none` body described under "UI conventions" below — the app renders, animates and logs perfectly while ignoring every click. An always-mounted trigger makes that unreachable rather than merely unlikely.
