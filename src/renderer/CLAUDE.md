...
**The naming rules live in `agent-core/dailyNote.js`, not here** — the coding agent's `daily_note` tool resolves the same files, and two copies would drift into each side opening a different note. `src/renderer/dailyNote.ts` is a re-export, the renderer's one door onto it. Slashes in the format become subfolder boundaries beneath the configured folder.
