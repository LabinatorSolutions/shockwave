...
- `CompanionUpdateDialog.tsx` (root of `src/renderer/`, rendered by both App.tsx and CompanionSection) — the remote-upgrade flow: confirm (versions + "in-flight Telegram/cron runs will be interrupted") → `apiUpgradeCompanion` → poll `apiCheckVersion` every 3s (up to 3 min; `unreachable` mid-poll is the expected container restart) → success toast on `match`. `updater-unavailable` (pre-sidecar companion) and timeout both surface the install one-liner. App.tsx runs the same check once on boot (mount effect): `companion-older` opens the dialog, `companion-newer` toasts "update the desktop app", everything else is silent.
