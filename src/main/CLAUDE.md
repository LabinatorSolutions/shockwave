...
**The decision itself is pure and unit-tested** — `src/main/api/certPolicy.js` (`decideCert`, `toDisplayFingerprint`, `pendingApplies`), covered by `tests/certPolicy.test.js`. `net.ts` is only the Electron wiring around it. Same pure-`.js`-policy split as `keys.js` vs `settingsStore.ts`.
