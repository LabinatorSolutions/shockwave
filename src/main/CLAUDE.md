...
- **`app://remote/?url=<encoded>`** — a remote image referenced from a file or a chat bubble (`![](https://…)`). **`img-src` in `index.html` deliberately has no `https:`**, so the page cannot load third-party images at all; the renderer rewrites those URLs (`resolveImageUrl` in `imageWidgets.ts`, used by both the editor widget and `ChatSidebar`) and main fetches them instead. A workspace file is content the agent writes and sync pulls in, so a page allowed to load arbitrary hosts is a beacon that fires on preview — proxying means the origin server sees this process and never the renderer's session. Only `http(s):` targets and only `image/*` responses come back (`415` otherwise); cached a day, not `immutable` like attachment ids. **The URL travels as a query param, not a path segment** — the `app` scheme is `standard: true`, so a percent-encoded URL in the path is at the mercy of Chromium's path normalization; the query is preserved verbatim.
...
- `builtinDir` (bundled built-in skills), `machine: os.hostname()`, `extraTools: [OPEN_FILE_TOOL, SEND_MESSAGE_TOOL]`, `extraTools` as a FUNCTION of the session (`({chatId}) => [open_file, send_message]`), because the host is built once per process while `send_message` needs the CHAT: each bubble is recorded against the conversation that sent it, so replying to it on Telegram continues *that* conversation. (It used to carry the workspace too, for the reply mode — that is app-level now and the companion reads it at the delivery end.) `dataDir: () => app.getPath('userData')` (one global pi scratch dir — the companion uses a per-run dir instead), `scratchDir: (chatId) => <userData>/agent-scratch/<chatId>` (the AGENT's own directory, named in the prompt — distinct from `dataDir`, which is pi's working memory).
...
### The companion's version rides the same signal — and gates writes

The desktop and the companion image are cut from **one release tag**, so a version mismatch means the two sides disagree about the shape of the data they exchange. That is not a nag; it is a reason to stop writing. Three pieces, and the shape of them is the point:

1. **One probe.** `onFeedOpen` health-checks and calls `classifyVersions` on **every** feed open. That is complete coverage with no polling, because the answer can only change when the companion *restarts* — and a restart drops the feed. Boot, any reconnect (including the companion coming back from its own upgrade), and Settings → Connect (which tears the feed down and starts it again) are all the same event.
2. **One latch, one push.** `setCompanionVersion` stores it, arms the kill switch, and broadcasts on the existing `companion:state`. **Going offline clears it** — a server we can't reach is one whose version we no longer know, and that is also what keeps an upgrade quiet: while the companion restarts into its new image the user sees the offline cloud alone, not a version warning stacked on top of it.
3. **One guard.** `setStaleCompanion` in `api/client.ts` makes `request()` refuse every non-GET. Reads stay open (a stale read destroys nothing, and an app showing what it has beats one pretending to be empty); writes are the whole risk. `health()` and `triggerUpdate()` deliberately bypass `request()`, which is the escape hatch — **the switch must never be able to block the two calls that turn it off.**

**`isCompanionStale` (`src/shared/constants.ts`) is the one predicate**, read by main and by four places in the renderer, for the same reason `agent-core/credentials.ts` is one declaration. It is true for `companion-older`/`companion-newer` and nothing else: `'dev'` means either side is unversioned — a local companion reports `APP_VERSION='dev'` — so treating it as stale would block every write in every development session. Pinned by `tests/versionCompare.test.js`.

> **This replaced three independent version checks**: a boot probe in `App.tsx` that opened the update dialog over a launching app, `CompanionSection`'s own probe that ran per-connect and was cleared on every edit, and this one. Three lifetimes over one server, so the settings page could read "up to date" while the toast said otherwise, and whichever you happened to be looking at was the one you believed. Adding a fourth reader is not the fix for the next gap — reading the latch is.
...
| Context menus | `context:fileMenu` (`conflictMode` → Conflict resolved / Keep our file / Reset to remote), `context:conflictCloudMenu` (whole-tree keep/reset), `context:folderMenu`, `context:editorMenu` (link add/edit/remove, Message Agent, and Reveal-in-Finder for the image under the pointer — each item is built only when the renderer says it applies, so `hasImagePath: false` omits the row rather than shipping one that can't work), `context:fallbackMenu` (cut/copy/paste roles for surfaces with no menu of their own — invoked by `main.tsx`'s document-level listener when no component preventDefaulted the right-click) |
| Settings | `settings:read`, `settings:write` (writes only the keys present), `settings:deleteCredential` (the only way to *remove* a stored credential — gated on `isDeletableCredential` from `agent-core/credentials.ts`, then `DELETE /settings/credential/:path` on the companion; clearing the input box deliberately cannot do this, see `settings/CLAUDE.md`), `settings:deleteAgentSecret` (likewise the only way to remove an agent secret — dropping a name from a saved list no longer deletes it, because this window's copy of the list is legitimately stale; see the boxed rule in `api/CLAUDE.md`); plus push event `settings:changed` (`{keys, settings}`, main-initiated writes only) |
...
| Agent | `agent:send` (takes `chatId`; steers if that chat is mid-turn), `agent:stashFiles` (composer attachments → the chat's scratch pad, answering a descriptor per file plus whether the model can see images), `agent:abort` (per chatId), `agent:runningChats`, `agent:listProviders`, `agent:listModels`, `agent:listThinkingLevels`, `agent:validateConnection` (checks an openai-compatible `{baseUrl}/models`); plus push events `agent:event` / `agent:error` (stamped with `chatId`) |
...
| Workspaces | `workspace:inspectFolder`, `workspace:createWithRepo`, `workspace:addFromRepo` (covers both clone-into-empty and adopt-a-clone), `workspace:setUpHere`, `workspace:remove`, `workspace:forgetLocal`, `workspace:listFiles`, `workspace:ensureFiles`. **There is no `workspace:setVoiceReply`** — the Telegram reply mode is an ordinary synced setting (`speech.telegramReply`) and rides `PATCH /settings` like everything else. It was a per-workspace write down its own channel until 2026-08-06, which meant the settings page wrote the mode of whichever workspace the DESKTOP had open while the bot obeyed its own — see `agent-core/voiceReply.ts`. |
...
| Companion config | `api:read`, `api:write`, `api:test` (the "connect to your server" form — URL + key, key `safeStorage`-wrapped; `api:test` also returns the companion's release version), `api:upgradeCompanion` (`POST /update` with this desktop's version — the companion's updater sidecar does the pull + restart; fire-and-forget, sets `pendingUpgradeTag`; **callable at any version, in either direction** — it is the way out of a mismatch, so it must never sit behind the switch a mismatch throws); the certificate trio `api:pendingCert` (what identity is being offered + what was approved before, `null` ⇒ first connection), `api:approveCert` (**the one place a fingerprint is ever stored**), `api:forgetCert`; `companion:getState` (reachability **and** version); push events `api:companionUpdated` (the live feed reconnected on the requested version — the feed's `onOpen` fires when the stream's HTTP response arrives, NOT on the first event, so a quiet server still announces), `companion:state` (reachability or version changed — see "Connection state" below), and **`api:companionChanged`** (the URL now points at a DIFFERENT server, so the renderer drops its settings, workspace list, open workspace and chats — see "Pointing at a different companion" in `src/renderer/CLAUDE.md`; **the key changing is not a switch**, the companion being single-tenant, and it is deliberately not the same signal as going offline). **There is no `api:checkVersion`**: the classification has one writer (`onFeedOpen`) and travels on `companion:state`, so no caller can ask for its own answer. |
