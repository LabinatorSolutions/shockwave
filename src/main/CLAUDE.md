...
- `builtinDir` (bundled built-in skills), `machine: os.hostname()`, `extraTools: [OPEN_FILE_TOOL, SEND_MESSAGE_TOOL]`, `extraTools` as a FUNCTION of the session (`({chatId, workspaceId}) => [open_file, send_message]`), because the host is built once per process while `send_message` needs both: the reply mode it reads is per WORKSPACE, and the chat is recorded against each bubble so replying to it on Telegram continues *that* conversation, `dataDir: () => app.getPath('userData')` (one global pi scratch dir — the companion uses a per-run dir instead), `scratchDir: (chatId) => <userData>/agent-scratch/<chatId>` (the AGENT's own directory, named in the prompt — distinct from `dataDir`, which is pi's working memory).
...
| Workspaces | `workspace:inspectFolder`, `workspace:createWithRepo`, `workspace:addFromRepo` (covers both clone-into-empty and adopt-a-clone), `workspace:setUpHere`, `workspace:remove`, `workspace:forgetLocal`, `workspace:listFiles`, `workspace:ensureFiles`, `workspace:setVoiceReply` (the Telegram-replies mode — `text` \| `voice` \| `both`; → `POST /workspaces/:id/voice`, then `notifyWorkspacesChanged()` so the renderer's copy follows. It is **per-workspace and lives on the companion**, not in `Settings`, because `/voice` in the bot writes the same column with no desktop involved — see `agent-core/voiceReply.ts`) |
