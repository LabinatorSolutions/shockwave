...
ipcMain.handle('voice:getToken', async () => {
  // Safe read with the real reason. This is prefetched on mount, so when it threw
  // — which an unreachable or un-approved companion made it do — every launch
  // logged an unhandled rejection in the renderer. And "not configured" would be
  // a lie when the key is configured and the server simply isn't reachable.
