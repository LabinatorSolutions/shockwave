...
| Attachments (any file the user sends, on either host) | `agent-core/{attachmentPolicy,attachmentNotes}.ts`, `src/renderer/{chatAttachments.ts,ChatSidebar.tsx}`, `src/main/codingAgent.ts` (`stashChatFile`), `api/src/telegram/attachments.ts` | the attachment entries in `agent-core/CLAUDE.md` |
