...
| Reviews (the agent writes its own skills after enough work) | `api/src/review{Sweeper,Run}.ts`, `agent-core/skill{Manage,Validate,Tool}.ts`, `agent-core/fuzzyMatch.ts`, `agent-core/defaults/reviewPrompt.ts` | "Reviews" in `api/CLAUDE.md` |
