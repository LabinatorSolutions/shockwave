...
- `telegram/transcribe.ts` — a thin adapter over `agent-core/transcribe.ts` for voice notes: bytes to a temp file, text back out. One speech-to-text implementation, shared with the agent's `transcribe` tool, using the same `transcription.apiKey` the desktop mic uses. Telegram sends OGG/Opus, which AssemblyAI takes as-is.
