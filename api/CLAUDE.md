...
- `reviewSweeper.ts` — the self-improvement trigger: a croner tick that finds a chat which has done enough work and reviews it. See "Self-improvement" below.
...
- `telegram/btw.ts` — `/btw <question>`: one short model call over the chat's stored messages. Not a turn — it never steers, never joins the conversation, touches no files, and works WHILE a job is running (which is the point). It can see an in-flight job only because messages are stored as pi completes each one. **It renders what the agent DID, not only what it said**: each tool call carries its arguments and each result a clipped head of its output, both already on the row (`tool_calls` / `content`) and both formerly discarded — rendering every call as a bare `[ran bash]` left it unable to answer "which directory / what command / what did it find" from data it had already fetched. Bounded newest-first by a whole-transcript character budget, so a long working turn trims its oldest rows and keeps the recent detail. **`get_agent_secret`'s output is named and never quoted** (`SECRET_OUTPUT_TOOLS`) — the result is a usable token and this answer goes out as a Telegram message; the *call* is still shown, since the secret's name is not the secret.
...
- **chat** / **message** — chats: session metadata + `source` (`desktop|cron|telegram|review`)/`source_id`/`machine` provenance + `running`/`running_machine` cross-client flag + `last_reviewed_seq` (how far self-improvement has looked — see below), plus the whole pi JSONL in a **`transcript` column** (it was a 1:1 `chat_transcript` side table, which bought nothing — Postgres TOASTs a big text column out of line and never reads it unless selected). `message` holds one row per pi session ENTRY, appended as pi completes it; identity is `entry_id` (pi's own id), and `seq` is an ordering/read cursor **assigned by the server**. It also carries a GENERATED `search_text` tsvector (user+assistant content only — tool output is deliberately unindexed) with a GIN index, backing the agent's `search_chats` tool.
...
## Self-improvement (`reviewSweeper.ts`, `reviewRun.ts`)
