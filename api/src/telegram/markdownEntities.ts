...
//
// CommonMark only, deliberately: the one dependency is the parser the desktop
// chat sidebar already runs (`mdast-util-from-markdown`, underneath
// react-markdown), so Telegram and the desktop interpret the agent identically.
// GFM tables and `~~strike~~` need further extensions and the helper prompt
// already tells the agent not to use either (`agent-core/defaults/helper.ts`,
// "Markdown supported"). A stray pipe table therefore arrives as plain text —
// what happens today — rather than pulling in a second parser for output the
// agent is told not to produce.
