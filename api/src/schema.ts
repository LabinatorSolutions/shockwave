...
export const workspace = pgTable('workspace', {
  ...
  // No `voice_reply` here any more: the Telegram reply mode is app-level and
  // lives in `setting` under `speech.telegramReply`. It sat on this row because
  // /voice is answered with no checkout prepared — a storage argument a settings
  // row satisfies just as well — while the SCOPE it implied was never intended.
  // See VOICE_REPLY_SETTING_PATH in agent-core/voiceReply.ts.
}, (t) => [
