...
Two workflows, **both triggered by the same `v*` tag**, and that shared trigger is the design: a desktop release and the companion image it talks to are one version by construction, so the desktop's "your companion is behind" check (`versionCompare.ts`) compares two numbers that came from the same commit.
...
## `release.yml` — desktop installers

Matrix build on macOS + Windows + Linux, then a separate `publish` job.
...
- **Publishing is atomic, and that's what the second job buys.** `package.json`'s `build.publish.releaseType` is **`draft`**, so every platform uploads into a draft; `publish` (with `needs: release`, i.e. all three succeeded) flips it public. electron-updater only reads *published* releases, so clients cannot see a release until every artifact is there. It used to be `releaseType: "release"` and went public the moment the first platform finished — while `latest-mac.yml`, written last after ~10 minutes of notarization, was still missing, so Mac clients polling in that window got a 404 on the update feed. A failed platform now leaves an unpublished draft to inspect rather than a half-published release. *(The comment at the top of the file still says `releaseType = "release"` — it predates this change; the later block is the accurate one.)*
...
## `companion-image.yml` — the companion container
