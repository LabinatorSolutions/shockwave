// Writing skills (agent-core/skillManage.ts + skillValidate.ts) — the one
...
import { skillManage } from '../agent-core/skillManage.ts';
import {
...
async function makeRoots() {
  ...
  const base = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), 'skillmanage-')));
...
test('create writes into the agent directory', async () => {
  ...
  const r = await skillManage(t.roots, { action: 'create', name: 'pdf-tools', content: SKILL('pdf-tools') });
...
test('GUARD: create refuses a name a built-in already uses', async () => {
  ...
  const r = await skillManage(t.roots, { action: 'create', name: 'firecrawl', content: SKILL('firecrawl') });
  assert.equal(r.success, false);
  assert.match(r.error, /already exists/);
});
...
test('create rejects a frontmatter name that disagrees with the skill name', async () => {
  const t = await makeRoots();
  const r = await skillManage(t.roots, { action: 'create', name: 'alpha', content: SKILL('beta') });
  assert.equal(r.success, false);
...
test("GUARD: edit refuses a skill in the user's uploaded directory", async () => {
  ...
  const r = await skillManage(t.roots, { action: 'edit', name: 'notes', content: SKILL('notes', 'Rewritten.') });
  assert.equal(r.success, false);
...
test('GUARD: patch refuses a built-in skill', async () => {
  ...
  const r = await skillManage(t.roots, {
...
test('write_file and remove_file round-trip, and tidy the emptied folder', async () => {
  const t = await makeRoots();
  await t.seed(t.agentDir, 'pdf-tools');
  const w = await skillManage(t.roots, {
...
test('editing a skill that does not exist says so', async () => {
  const t = await makeRoots();
  const r = await skillManage(t.roots, { action: 'edit', name: 'nope', content: SKILL('nope') });
  assert.equal(r.success, false);
...
test('there is no delete action', async () => {
  const t = await makeRoots();
  await t.seed(t.agentDir, 'pdf-tools');
  const r = await skillManage(t.roots, { action: 'delete', name: 'pdf-tools' });
  assert.equal(r.success, false);
